...
export class ChangePasswordComponent implements OnInit {
  ...
  newOldPasswordChange() {
    return (control: AbstractControl) => {
      if (this.validateForm) {
        const p1 = this.validateForm.controls["password"];
        const p2 = this.validateForm.controls["password1"];
        if (p1.value === p2.value) {
          return {
            newOldPasswordChange: true
          }
        }
      }
      return null;
  ...
  private errorTip(c: AbstractControl) {
    if (c.hasError("minlength") || c.hasError("maxlength")) {
      ...
      return "两次密码输入不一致";
    } else if (c.hasError("newOldPasswordChange")) {
      return "新密码和旧密码不能相同";
    }
    return null;
  }
